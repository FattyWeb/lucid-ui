<template>
  <div class="code" ref="codebox">
    <div
      :class="{'copied': check}"
      class="noti-code"
    >
      <i class='bx bx-check' ></i> 代码已复制
    </div>
...
export default {
  ...
  methods:{
    ...
    copy(){
      ...
      // todo 此处算法有问题
      const codeBox = this.$refs.codebox.querySelectorAll('div[class*="language-"] pre');
      let text = this.activeSlot == 3 ? (()=>{
        let tempText = ''
        for (let i = 1; i < codeBox.length ; i++) {
          tempText += codeBox[i].innerText
        }
        return tempText;
      })(): codeBox[slot].innerText;
