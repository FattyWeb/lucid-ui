...
export default {
  ...
  mounted () {
    this.placeholder = useThemeData().value.searchPlaceholder || ''
  ...
  computed: {
    ...
    suggestions () {
      ...
      const max = this.$theme.searchMaxSuggestions || SEARCH_MAX_SUGGESTIONS
      const localePath = this.$routeLocale
      const matches = item => (
        item
        && item.title
    ...
    alignRight () {
      const navCount = (this.$theme.nav || []).length
