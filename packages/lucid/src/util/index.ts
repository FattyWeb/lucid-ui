...
// • Warn info: El formato de la propiedad color en incorrecto
// • Prop: color
// • value: ${color}
...
const moveLeft = (element: any, parent: any, ) => {
  const style = element.style
  const cords = parent.getBoundingClientRect()
  const w = cords.width
  const x = cords.x
  style.left = `${x - (element.offsetWidth - w) - (element.offsetWidth / 2)}px`
}

const setCords = (element: any, parent: any, options?:any) => {
  const cords = parent.getBoundingClientRect()
  const x = cords.x
  const y = cords.y
  const w = cords.width
  ...
  if ((rootTop - elTop) < 30) {
    ...
    style.top = `${y + scrollTop - element.clientHeight - 4 - ((options && options.margin) ? options.margin : 0 )}px`
    style.left = `${x - ((options && options.center) ? ((element.offsetWidth - w) / 2) : 0)}px`
    style.width = (options && options.ignoreWidth) ? 'auto' : `${w}px`
    element.classList.add('top')
    parent.classList.add('top')
  } else {
    style.top = `${y + scrollTop + h - 4 + ((options && options.margin) ? options.margin : 0 )}px`
    style.left = `${x - ((options && options.center) ? ((element.offsetWidth - w) / 2) : 0)}px`
    style.width = (options && options.ignoreWidth) ? 'auto' : `${w}px`
...
/**
 * 将vue3的$attrs 分割为vue2的 $attrs + $listeners
 *
 * @param attrs
 */
const splitAttrsInVue2 = (attrs: object): { listeners: object, attrs: object} => {
  const split = {
    listeners: {},
    attrs: {}
  }
  for (const [k, v] of Object.entries(attrs)) {
    (split[(k.startsWith('on') && typeof v === 'function') ? 'listeners' : 'attrs'] ||= {})[k] = v;
  }
  return split;
}

const excludeClassStyleAttrs = (attrs: object) => {
  const exclude = {}
  for (const [k, v] of Object.entries(attrs)) {
    (!['class', 'style'].includes(k)) && (exclude[k] = v)
  }
  return exclude;
}
